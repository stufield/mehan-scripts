"LinearStabilitySelectionTest1" <- function(n=100, p=1000, sigma=.1)
{  
  n1 = floor(n/2)
  n2 = n-n1-1;
...
"LinearStabilitySelection" <- function(dFrame,y,alpha=0.85,Pw=0,nbootstrap=100,nsteps=-1, plotme=TRUE, lamMax=-1)
{
	# Stability selection in the spirit of Meinshausen&Buhlman using lars for Linear Reg
  # Input: 
  ...
  #
  # Note: Lars package doesn't allow user to choose steps so in general each regularization path will have different points!
	require(lars)
 	n <- dim(dFrame)[1]
	p <- dim(dFrame)[2]
  ...
  lars.fit = lars(as.matrix(dFrame),y, max.steps=nsteps, use.Gram=FALSE)
  if ( lamMax == -1) {
    lam=lars.fit$lambda;  
  }
  else{
  ...
  lambdaMax = vector();
  #lambdaMat = matrix(0,nbootstrap,nsteps); 
  X = as.matrix(dFrame);
	for (i in seq(nbootstrap)) {
      ...
      lars.fit = lars(Xs[i1,],y[i1], max.steps = nsteps, use.Gram=FALSE,normalize=F)
      #lambdaMat[i,] = lars.fit$lambda / lars.fit$lambda[1]; # Store the regulaization path so we can check that they are the same...
      lambdaMax = c(lambdaMax, lars.fit$lambda[1]);
    
      # Find the the variable that were selected at some point on the path as those with non-zero coefficients
  		tmp = as.matrix(lars.fit$beta)    # nsteps x p
  		sel.index = (1:dim(Xs)[2])[lars.fit$entry>0]  # Column indices for selected variables
      ...
      lars.fit = lars(Xs[i2,],y[i2], max.steps=nsteps, use.Gram=FALSE, normalize=F)  
      lambdaMax = c(lambdaMax, lars.fit$lambda[1]);
 ...
 result <- list(freq=freq, lambda=lam, lambdaMatrix = lambdaMat, selectionProb=apply(freq,2,max), numSamples=2*nbootstrap, nsteps = nsteps, alpha=alpha, Pw = Pw,lambdaMax = lambdaMax)
...
#
#
"LinearStabilityPlot" <- function(object, targetNames=colnames(object$freq), labelsize = 0.6, pLabelThresh=0.5, ymax=1, normalize=FALSE,...)
{
    freq   = object$freq;
    if ( normalize ) {
